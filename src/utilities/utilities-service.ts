 ...
 * @param storeData : any
 */
export function setStoreData(context: vscode.ExtensionContext, storeData: any, key: string) {
 ...
 * @returns string
 */
export function getStoreData(context: vscode.ExtensionContext, key: string): any {
    const state = stateManager(context);
 ...
 * Uri for history
 */
export function getHistoryUri(context: vscode.ExtensionContext, pathList: string[]) {
  return vscode.Uri.joinPath(context.globalStorageUri, "scriptiq_history", ...pathList); // Also use name in full-test-gen-view-panel.ts
}
