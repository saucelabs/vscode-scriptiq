...
import { setStoreData, getStoreData, getHistoryUri, getScreenshotUri } from './utilities/utilities-service';
...
export function activate(context: vscode.ExtensionContext) {
	...
	console.log('Congratulations, your extension "sauce-scriptiq" is now active!');
	...
	const testGenerationPanelCommand = vscode.commands.registerCommand("testGeneration.start", () => {	
		TestGenerationPanel.render(context);
	});
	context.subscriptions.push(testGenerationPanelCommand);
	
	const testLoadHistoryCommand = vscode.commands.registerCommand("testLoadHistory.start", () => {
	...
	const updateHistoryLinksCommand = vscode.commands.registerCommand("updateHistoryLinks.start", () => {
		provider.updateHistoryLinks();
	})
	context.subscriptions.push(updateHistoryLinksCommand);

	const updateHistoryLinksNewTestCommand = vscode.commands.registerCommand("updateHistoryLinksNewTest.start", () => {
		provider.updateHistoryLinksNewTest();
	})
	context.subscriptions.push(updateHistoryLinksNewTestCommand);

	const clearHistoryLinkSelectionCommand = vscode.commands.registerCommand("clearHistoryLinkSelection.start", () => {
